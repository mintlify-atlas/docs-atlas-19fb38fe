...
graph TB
    ...
    API --> LanceDB[LanceDB<br/>Local Vector Store]
```
...
- **Qdrant**: Vector database for storing document embeddings (Docker deployment)
- **LanceDB**: Local vector storage alternative (Development mode)
...
<AccordionGroup>
  <Accordion title="Use Docker for Production">
    ...
    - Complete isolation with Qdrant vector database included
    
    All services run in containers with proper networking and volume management.
  </Accordion>
